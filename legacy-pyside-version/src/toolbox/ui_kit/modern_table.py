...
class ModernTableWidget(QTableWidget):
    ...
    def setup_table(self):
        ...
        # 행 높이 - 반응형 스케일링 적용
        row_height = max(30, int(35 * scale))  # 최소 30px 보장
    ...
    def setup_styling(self):
        ...
        self.setStyleSheet(f"""
            ...
            /* 체크박스 스타일 - 반응형 스케일링 적용 */
            QTableWidget::indicator {{
                width: {checkbox_size}px;
                height: {checkbox_size}px;
                border: {max(1, int(2 * scale))}px solid #ccc;
                border-radius: {max(1, int(3 * scale))}px;
    ...
    def setup_header_checkbox(self):
        ...
        # 실제 체크박스 위젯 생성 (개별 체크박스와 동일한 스타일) - 반응형 스케일링 적용
        self.header_checkbox = QCheckBox()
        self.header_checkbox.setFocusPolicy(Qt.NoFocus)  # 포커스 표시 제거
        
        # 스케일링 적용
        scale = tokens.get_screen_scale_factor()
        checkbox_size = max(1, int(16 * scale))  # 최소 1px 보장
        border_width = max(1, int(2 * scale))   # 최소 1px 보장
        border_radius = max(1, int(3 * scale))  # 최소 1px 보장
        margin = max(1, int(2 * scale))         # 최소 1px 보장
        ...
        # 헤더에 체크박스 위젯을 오버레이로 배치 - 스케일링 적용
        self.header_checkbox.setParent(self.horizontalHeader())
        header_x = max(8, int(11 * scale))  # x 위치 스케일링
        header_y = max(6, int(8 * scale))   # y 위치 스케일링
        self.header_checkbox.move(header_x, header_y)
