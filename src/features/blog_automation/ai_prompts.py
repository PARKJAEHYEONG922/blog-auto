...
class BlogSummaryPrompts:
    ...
    def generate_content_summary_prompt(combined_content: str, main_keyword: str, content_type: str = "정보/가이드형") -> str:
        ...
        purpose_by_type = {
            "후기/리뷰형": {
                "purpose": "실제 사용자들의 후기와 경험담을 바탕으로 더 나은 리뷰 콘텐츠를 작성하기 위해",
                "focus": "사용자 경험, 장단점, 구매/사용 결정 요인, 실제 후기 내용"
            },
            "정보/가이드형": {
                "purpose": "독자들에게 더 유용하고 완벽한 가이드를 제공하기 위해",
                "focus": "핵심 정보, 단계별 방법, 주의사항, 팁과 노하우, 전문 지식"
            },
            "비교/추천형": {
                "purpose": "독자의 선택을 돕는 객관적이고 유용한 비교 콘텐츠를 작성하기 위해",
                "focus": "비교 기준, 각 옵션의 특징, 상황별 추천 포인트, 선택 가이드"
            }
        }
        
        type_info = purpose_by_type.get(content_type, purpose_by_type["정보/가이드형"])
        
        # 1차 가공 프롬프트 (정보요약 AI용)
        summary_prompt = f"""### 네이버 블로그 콘텐츠 분석 및 요약 작업

**목적**: 네이버 블로그에 '{main_keyword}' 키워드로 글을 작성할 예정입니다. 
현재 이 키워드로 상위 노출된 경쟁 블로그들의 콘텐츠를 분석하여, 
다음 단계에서 글작성 AI가 참고할 수 있도록 핵심 정보를 정리해주세요.

**콘텐츠 유형**: {content_type}
**분석 목적**: {type_info['purpose']}

**요약 시 중점적으로 추출해야 할 정보**:
- {type_info['focus']}
- 네이버 블로그 SEO에 효과적인 키워드 패턴
- 독자들이 자주 찾는 핵심 질문과 답변
- 경쟁 블로그들이 공통적으로 다루는 필수 포인트
- 차별화할 수 있는 부족한 정보나 개선점

**요약 구조**:
1. **핵심 키워드 & 주제**: 주요 검색 키워드와 핵심 주제 정리
2. **필수 다뤄야 할 내용**: 모든 블로그가 공통으로 언급하는 핵심 정보
3. **독자가 궁금해하는 점**: 자주 등장하는 질문이나 관심사
4. **차별화 포인트**: 기존 글들이 놓친 부분이나 개선 가능한 영역
5. **SEO 최적화 요소**: 효과적인 키워드 사용법이나 구조적 특징
...
위 경쟁 분석을 바탕으로 글작성 AI가 더 나은 '{main_keyword}' 관련 {content_type} 콘텐츠를 작성할 수 있도록 체계적으로 정리해주세요. 
단순 요약이 아닌, 실전에서 활용 가능한 인사이트 중심으로 작성해주세요."""
...
2. **글자 수**: 최소 1,500자 이상 작성 필수입니다. 상위 노출된 1~3위 블로그 포스팅의 평균 글자 수({summary.get('avg_content_length', 1500)}자)보다 더 풍부하고 상세하게 작성해 주세요. 이상적으로는 1,800자에서 2,500자(공백 제외) 사이로 충분히 길게 작성하여 검색 알고리즘에 유리하도록 해주세요
...
- [ ] 최소 1,500자 이상, 이상적으로는 1,800~2,500자 범위 작성 (공백 제외)
