...
class MainWindow(QMainWindow):
    ...
    def on_api_settings_changed(self):
        ...
        try:
            ...
            # 모든 활성화된 feature 위젯에 API 설정 변경 시그널 브로드캐스트
            self.broadcast_api_settings_changed()
            
        except Exception as e:
            logger.error(f"API 설정 변경 처리 오류: {e}")
    
    def broadcast_api_settings_changed(self):
        """모든 활성화된 feature 위젯에 API 설정 변경 시그널 브로드캐스트"""
        try:
            logger.info("모든 활성화된 모듈에 API 설정 변경 시그널 브로드캐스트")
            
            for widget_id, widget in self.feature_widgets.items():
                try:
                    # 위젯에 _on_api_settings_changed 메서드가 있으면 호출
                    if hasattr(widget, '_on_api_settings_changed'):
                        logger.info(f"API 설정 변경 시그널 전송: {widget_id}")
                        widget._on_api_settings_changed()
                except Exception as e:
                    logger.error(f"위젯 {widget_id}에 API 설정 변경 시그널 전송 실패: {e}")
                    
        except Exception as e:
            logger.error(f"API 설정 변경 브로드캐스트 오류: {e}")
    ...
    def load_page(self, page_id):
        ...
        try:
            ...
            # 특정 모듈의 위젯 동적 로드
            if page_id == 'neighbor_add':
                from src.features.neighbor_add.ui_main import NeighborAddMainUI
                widget = NeighborAddMainUI()
                self.feature_widgets[page_id] = widget
                return widget
            elif page_id == 'blog_automation':
                from src.features.blog_automation.ui_main import BlogAutomationMainUI
                widget = BlogAutomationMainUI()
                self.feature_widgets[page_id] = widget
                return widget
            elif page_id == 'powerlink_automation':
                from src.features.powerlink_automation.ui_main import PowerlinkAutomationWidget
                widget = PowerlinkAutomationWidget()
                self.feature_widgets[page_id] = widget
                return widget
            elif page_id == 'keyword_analysis':
                from src.features.keyword_analysis.ui_main import KeywordAnalysisWidget
                widget = KeywordAnalysisWidget()
                self.feature_widgets[page_id] = widget
                return widget
            elif page_id == 'powerlink_analyzer':
                from src.features.powerlink_analyzer.ui_main import PowerLinkAnalyzerWidget
    ...
    def get_module_name(self, module_id):
        """모듈 ID에서 이름 가져오기"""
        module_names = {
            'neighbor_add': '서로이웃 추가',
            'blog_automation': '블로그 자동화',
    ...
    def title_to_page_id(self, title):
        ...
        title_map = {
            ...
            '파워링크 자동입찰': 'powerlink_automation',
