...
export class YouTubeAPI {
  ...
  // youtube-transcript로 자막 추출 (간단한 버전)
  ...
  // 간단한 자막 추출 (300자 이상만 통과)
  async extractSubtitlesSimple(videoId: string): Promise<SubtitleTrack[]> {
    try {
      console.log(`📝 자막 추출: ${videoId}`);

      // 한국어 자막 시도
      let subtitleText = await this.fetchSubtitleText(videoId, 'ko');
      let language = 'ko';

      // 한국어가 없거나 너무 짧으면 영어 시도
      if (!subtitleText || subtitleText.length < 300) {
        console.log(`한국어 자막 부족 (${subtitleText ? subtitleText.length : 0}자), 영어 시도`);
        subtitleText = await this.fetchSubtitleText(videoId, 'en');
        language = 'en';
      }

      // 300자 이상인지 확인
      if (!subtitleText || subtitleText.length < 300) {
        console.warn(`자막이 너무 짧음: ${subtitleText ? subtitleText.length : 0}자 (최소 300자 필요)`);
        return [];
      }

      console.log(`✅ 자막 추출 성공: ${language} ${subtitleText.length}자`);
